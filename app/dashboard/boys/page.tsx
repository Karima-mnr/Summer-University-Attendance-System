...
export default function BoysPage() {
  ...
  const fetchParticipants = async () => {
    try {
      ...
      const boys = data.filter((p: IParticipant) => p.gender === 'Boy');
      setParticipants(boys);
    } catch (error) {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-3">
            <Users size={28} className="text-[#0B6B3A]" />
            <div>
              <h1 className="text-2xl font-bold text-slate-900">{t.boys}</h1>
              <p className="text-sm text-slate-500">{t.manageMale}</p>
            </div>
          </div>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="flex items-center gap-2 rounded-2xl bg-gradient-to-br from-[#d5f3e4e8] to-[#caf6ddee] px-6 py-3 text-sm font-semibold text-green-900 shadow-lg shadow-[#0B6B3A]/10 transition-all hover:-translate-y-0.5 hover:shadow-lg"
        >
          <UserPlus size={18} />
          {t.addMember}
        </button>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-slate-200 bg-white/80 py-3 pl-12 pr-12 text-sm shadow-sm backdrop-blur-sm transition-all focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
          />
          {searchQuery && (
            <button
              onClick={clearSearch}
